...
data {
    ...
    vector[N] p1_age;
    vector[N] p2_age;
    int<lower = 0, upper = 1> outcome[N]; // Results. 1 if player 1 won, 0 if player 2 won.
}
...
model {
    ...
    for (i in 1:N){
        // Using the Bernoulli_logit allows us to directly compute the outcome without the additional step of using the inverse_logit.
        outcome[i] ~ bernoulli_logit(p1_skill[i] - p2_skill[i]);
    }
}
